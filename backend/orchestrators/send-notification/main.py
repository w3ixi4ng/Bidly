...
channel.queue_declare(queue="auction_end_notifications", durable=True)
channel.queue_bind(
    exchange="bidly",
    queue="auction_end_notifications",
    routing_key="end.auction.notifications"
)

channel.queue_declare(queue="bid_outbid_notifications", durable=True)
channel.queue_bind(
    exchange="bidly",
    queue="bid_outbid_notifications",
    routing_key="out.bidded.notifications"
)

channel.basic_qos(prefetch_count=10)
channel.basic_consume(queue="auction_end_notifications", on_message_callback=on_auction_end_message, auto_ack=False)
channel.basic_consume(queue="bid_outbid_notifications", on_message_callback=on_bid_outbid_message, auto_ack=False)
